...
class Cpu {
  ...
  interrupt(vectorAddress) {
    ...
    debug('PC value', vectorAddress, this.registerPC);
  ...
  ROL(instruction) {
    ...
    if (instruction.mode === 'accumulator') {
      const oldRegisterC = this.registerC;
      this.registerC = (this.registerA >>> 7) & 1;
      this.registerA = (this.registerA << 1) | oldRegisterC;
      this.setZeroAndNegativeFlag(this.registerA);
      //TODO set carry flag
    } else {
      const oldRegisterC = this.registerC;
      let value = this.memory.read8(instruction.address);
      this.registerC = (value >>> 7) & 1;
      value = (value << 1) | oldRegisterC;
      this.memory.write8(instruction.address, value);
      this.setZeroAndNegativeFlag(value);
    }
  }
  ...
  ROR(instruction) {
    ...
    if (instruction.mode === 'accumulator') {
      const oldRegisterC = this.registerC;
      this.registerC = this.registerA & 1;
      this.registerA = (this.registerA >>> 1) | (oldRegisterC << 7);
      this.setZeroAndNegativeFlag(this.registerA);
      //TODO set carry flag
    } else {
      const oldRegisterC = this.registerC;
      let value = this.memory.read8(instruction.address);
      this.registerC = value & 1;
      value = (value >>> 1) | (oldRegisterC << 7);
      this.memory.write8(instruction.address, value);
      this.setZeroAndNegativeFlag(value);
    }
  }
