...
class Cpu {
  ...
  constructor(memory) {
    ...
    this.registerY = 0;
    //Processor Status, 8b
    this.registerP = Cpu.updateRegisterP(0);
  ...
  reset() {
    ...
    // TODO 0x24 or 0x34?
    this.registerP = Cpu.updateRegisterP(0x24);
    debug('%o', this);
  }
  ...
  triggerNMI() {
    this.interruptPending = VECTOR_ADDR_NMI;
  }

  triggerIRQ() {
    if (!this.interruptPending) {
      this.interruptPending = VECTOR_ADDR_IRQ;
      return;
    }
  ...
  pushStack8(value) {
    const stackPointer = (this.registerSP & 0xff) | memoryMap.MEMORY_OFFSET_STACK.offset;
    //debug('push stack at pos', stackPointer);
    this.memory.write8(stackPointer, value);
    this.registerSP = (this.registerSP - 1) & 0xff;
  }
  ...
  setFlagsCompare(register, value) {
    const temp = (register - value) & 0xff;
    this.setZeroAndNegativeFlag(temp);
    // If the value in the register is equal or greater than the compared value, the Carry will be set
    this.registerP.carry = register >= value;
  }
  ...
  getNextInstruction() {
    ...
    return { instruction, address: result, opcode };
  }
  ...
  executeCycle() {
    ...
    //check for irq
    if (this.interruptPending) {
      debug('IRQ pending', this.interruptPending);
    ...
    if (!instruction.size) {
      ...
      throw new Error('INVALID_OPCODE_'+instruction.name);
    }
  ...
  calculateAdc(value) {
    let temp = this.registerA + value + this.registerP.carry;
    this.registerP.carry = temp > 0xff;
    temp &= 0xff;
    // The overflow flag is set when the sign of the addends is the same and differs from the sign of the sum
    this.registerP.overflow = !!((this.registerA ^ temp) & (value ^ temp) & 0x80);
    this.setZeroAndNegativeFlag(temp);
    this.registerA = temp;
  }
  ...
  BRK() {
    ...
    //the quirk
    if (this.interruptPending === VECTOR_ADDR_NMI) {
  		this.interruptPending = undefined;
      this.registerPC = this.memory.read16(VECTOR_ADDR_NMI);
  	} else {
      this.registerPC = this.memory.read16(VECTOR_ADDR_IRQ);
  	}
  }
  ...
  SBC(instruction) {
    ...
    //console.log('sbc', instruction, subtractValue);
    let temp = this.registerA - subtractValue - (1 - this.registerP.carry);
    //console.log('temp',temp, this.registerP.carry);
    this.registerP.carry = temp >= 0;
    temp &= 0xff;
    // The overflow flag is set when the sign of the addends is the same and differs from the sign of the sum
    this.registerP.overflow = !!((this.registerA ^ temp) & (this.registerA ^ subtractValue) & 0x80);
    this.setZeroAndNegativeFlag(temp);
    this.registerA = temp;
  }
