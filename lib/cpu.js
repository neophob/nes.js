...
class Cpu {
  ...
  executeCycle() {
    ...
    const nextPC = (this.registerPC + 1) & 0xffff;

//TODO memory read here is very broken, fixme!

    switch (instruction.mode) {
    	case 'absolute':
    		address = this.memory.read16(nextPC);
        break;
    	case 'absoluteX':
    		address = this.memory.read16(nextPC) + this.registerX;
    		pageCrossed = this.pagesDiffer(address - this.registerX, address);
        break;
    	case 'absoluteY':
    		address = this.memory.read16(nextPC) + this.registerY;
        ...
        offset = this.memory.read8(nextPC);
        address = this.memory.read16Bug(offset + this.registerX);
        break;
    	case 'indirect':
        offset = this.memory.read16(nextPC);
        address = this.memory.read16Bug(offset);
        break;
        ...
        offset = this.memory.read8(nextPC) & 0xffff;
        ...
        //memory locations so they can take advantage of zero page addressing.
    		address = this.memory.read8(nextPC);
        break;
    	case 'zeroPageX':
    		address = this.memory.read8(nextPC) + this.registerX;
        break;
    	case 'zeroPageY':
    		address = this.memory.read8(nextPC) + this.registerY;
        break;
    }
