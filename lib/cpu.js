...
class Cpu {
  ...
  ADC(instruction) {
    ...
    const temp = this.registerA + addValue + this.registerP.carry;
    this.setZeroAndNegativeFlag(temp);
		this.registerP.overflow = !!((this.registerA ^ temp) & (addValue ^ temp) & 0x80) && 1;
  ...
  SBC(instruction) {
    ...
    // 6502BUG: The ADC and SBC instructions don't set N,V, and Z status bits if decimal
    // mode is on.  C status is set correctly.
    const subtractValue = this.memory.read8(instruction.address) ^ 0xff;
    const temp = this.registerA + subtractValue + this.registerP.carry;
    this.setZeroAndNegativeFlag(temp);
