...
class Mapper0 {
  ...
  _writePrg(offset, value, busSize) {
    ...
    if (busSize === 8) {
      return this.programRom.writeUInt8(offset);
    }
    return this.programRom.writeUInt16LE(offset);
  }

  _writeRam(offset, value, busSize) {
    debug('MEM write, offset %d, length %d', offset, this.programRom.length);
    if (busSize === 8) {
      return this.ram.writeUInt8(offset);
    }
    return this.ram.writeUInt16LE(offset);
  }

  write(offset, data, busSize) {
    debug('write %d at %d, size: %d', offset, data, busSize);
    const address = this._getAddress(offset);
    switch (address.subsystem) {
      case 'chr':
        ...
        break;
      case 'prg':
        this._writePrg(address.offset, data, busSize);
        break;
      case 'ram':
        this._writeRam(address.offset, data, busSize);
        break;
    }
  }
}
