...
class Mapper0 {
  ...
  constructor(rom) {
    ...
    this.mirrorType = rom.metaData.mirrorType;
  }
  ...
  _getAddress(offset) {
    ...
    if (offset >= MEMORY_OFFSET_PRG) {
      ...
      return this._buildReturnModel((offset - MEMORY_OFFSET_PRG) % 0x8000, 'prg');
    }
  ...
  _readRam(offset, busSize) {
    debug('MEM read, offset %d, length %d', offset, this.ram.length);
    if (busSize === 8) {
  ...
  _readChr(offset, busSize) {
    debug('CHR read, offset %d, length %d', offset, this.characterRom.length);
    if (busSize === 8) {
      return this.characterRom.readUInt8(offset);
    }
    return this.characterRom.readUInt16LE(offset);
  }
  ...
  read(offset, busSize) {
    ...
    switch (address.subsystem) {
      case 'chr':
        return this._readChr(address.offset, busSize);
      case 'prg':
  ...
  _writeRam(offset, value, busSize) {
    debug('MEM write, offset %d, length %d', offset, this.ram.length);
    if (busSize === 8) {
  ...
  _writeChr(offset, value, busSize) {
    debug('CHR write, offset %d, length %d', offset, this.characterRom.length);
    if (busSize === 8) {
      return this.characterRom.writeUInt8(value, offset);
    }
    return this.characterRom.writeUInt16LE(value, offset);
  }
  ...
  write(offset, value, busSize) {
    ...
    switch (address.subsystem) {
      case 'chr':
        this._writeChr(address.offset, value, busSize);
        break;
      case 'prg':
