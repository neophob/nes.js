...
module.exports.getAddress = function(cpu, instruction) {
  ...
  switch (instruction.mode) {
    ...
    case 'absoluteX':
      address = (cpu.memory.read16(nextPC) + cpu.registerX) & 0xffff;
      pageCrossed = cpu.pagesDiffer(address - cpu.registerX, address);
      return { address, pageCrossed };

    case 'absoluteY':
      address = (cpu.memory.read16(nextPC) + cpu.registerY) & 0xffff;
    ...
    case 'indirectIndexed':
      ...
      address = (cpu.memory.read16Bug(offset) + cpu.registerY) & 0xffff;
      pageCrossed = cpu.pagesDiffer(address - cpu.registerY, address);
      return { address, pageCrossed };
    ...
    case 'relative':
      ...
      address = (nextPC + 1 + offset) & 0xffff;
      return { address, pageCrossed };
