...
const NAMETABLE_VRAM_SIZE = 2048;
...
class Ppu {
  ...
  constructor() {
    ...
    this.nametableVramData = Buffer.from(new Uint8Array(NAMETABLE_VRAM_SIZE));
  ...
  fetchAttributeTableByte() {
    const address = 0x23C0 | (this.registerV & 0x0C00) | ((this.registerV >>> 4) & 0x38) | ((this.registerV >>> 2) & 0x07);
    const shift = ((this.registerV >>> 4) & 4) | (this.registerV & 2);
    this.attributeTableByte = ((this.readMemory(address) >>> shift) & 3) << 2;
  }

  fetchLowTileByte() {
    const fineY = (this.registerV >>> 12) & 7;
    const table = this.flagBackgroundTable;
    const tile = this.nameTableByte;
    const address = 0x1000 * table + tile * 16 + fineY;
    this.lowTileByte = this.readMemory(address);
  }
  ...
  fetchHighTileByte() {
    ...
    const tile = this.nameTableByte;
    const address = 0x1000 * table + tile * 16 + fineY;
    this.lowTileByte = this.readMemory(address + 8);
  }
  ...
  storeTileData() {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const p2 = (this.lowTileByte & 0x80) >>> 6;
      this.lowTileByte <<= 1;
      this.highTileByte <<= 1;
      data <<= 4;
      data |= this.attributeTableByte | p1 | p2;
    }
    this.tileData |= data;
  }
  ...
  evaluateSprites(){
    const h = this.flagSpriteOverflow === 0 ? 8 : 16;
    let count = 0;
    for (let i = 0; i < 64; i++) {
      const y = this.oamData[i*4+0];
  		const a = this.oamData[i*4+2];
  		const x = this.oamData[i*4+3];
  		const row = this.tv.scanLine - y;
  		if row < 0 || row >= h {
  			continue
  		}
  		if count < 8 {
...
/*      if (this.registerF === 1 && this.scanLine === 261 && this.cycle === 239) {
  ...
  readMemory(offset) {
    ...
    switch (address.subsystem) {
      ...
      case 'vram':
        return this.nametableVramData.readUInt8(address.offset);
      case 'palette':
  ...
  writeMemory(offset, data) {
    ...
    switch (address.subsystem) {
      ...
      case 'vram':
        return this.nametableVramData.writeUInt8(data, address.offset);
      case 'palette':
